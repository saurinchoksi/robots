...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        for i, link in enumerate(link_names):
            ...
            if i == 0:
                T_L0 = tf.transformations.translation_matrix(joints[i].origin.xyz)
                msgs.append(convert_to_message(T_L0, link_names[i], 'world_link'))

            else:

                for x, name in enumerate(joint_values.name):
                    if name == joints[i].name:
                        position = joint_values.position[x]
